...
model {
  ...
  for (t in 1:trials){
    ...
    rate = softmax(exp(logInvTemperature_mu + logInvTemperature_ID[agent]) * to_vector(Value));
    target += categorical_lpmf(choice[t,agent] | rate);//choice[t] ~ categorical(rate);
    
    predError = feedback[t,agent] - to_vector(Value);
    Value = to_vector(Value) + inv_logit(logit_learningRate_neg_mu + logit_learningRate_neg_ID[agent]) * predError * dummy_neg[t] + inv_logit(logit_learningRate_pos_mu + logit_learningRate_pos_ID[agent]) * predError * dummy_pos[t] ;
